...
func _validate() -> void:
	...
	if ammo_after >= ammo_before:
		...
		return
	if not bool(propulsion.get("uses_primary_fire_fuel", false)) or velocity_after.y < 3.0:
		_fail("flamethrower primary propulsion too weak or missing: %s" % str(propulsion))
		return
