...
- Host skal sætte et match-password før ENet-sessionen accepterer spillere i lobby/kamp. GUI `./run.sh --host` åbner lobbyen i host-mode, viser hostens public IP + port til deling, og lader spilleren vælge navn/loadout og indtaste password før `Start`. Direkte headless/automation-host kan bruge `--password=<password>`, `SHOOTER_MATCH_PASSWORD`, eller den skjulte `run.sh` terminalprompt.
